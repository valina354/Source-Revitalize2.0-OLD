...
#include "pp_teeth_vs20.inc"
#include "pp_teeth_flashlight_vs20.inc"
#include "pp_teeth_bump_vs20.inc"
#include "pp_teeth_ps20.inc"
#include "pp_teeth_ps20b.inc"
#include "pp_teeth_flashlight_ps20.inc"
#include "pp_teeth_flashlight_ps20b.inc"
#include "pp_teeth_bump_ps20.inc"
#include "pp_teeth_bump_ps20b.inc"

#ifndef _X360
#include "pp_teeth_vs30.inc"
#include "pp_teeth_ps30.inc"
#include "pp_teeth_bump_vs30.inc"
#include "pp_teeth_bump_ps30.inc"
#include "pp_teeth_flashlight_vs30.inc"
#include "pp_teeth_flashlight_ps30.inc"
#endif
		...
		SHADOW_STATE
		{
			...
			if ( hasBump )
			{
...
#endif
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( pp_teeth_bump_vs20 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER_COMBO( USE_STATIC_CONTROL_FLOW, bUseStaticControlFlow );
					SET_STATIC_VERTEX_SHADER( pp_teeth_bump_vs20 );

					// ps_2_b version which does phong
					if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( pp_teeth_bump_ps20b );
						SET_STATIC_PIXEL_SHADER( pp_teeth_bump_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( pp_teeth_bump_ps20 );
						SET_STATIC_PIXEL_SHADER( pp_teeth_bump_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER( pp_teeth_bump_vs30 );

					DECLARE_STATIC_PIXEL_SHADER( pp_teeth_bump_ps30 );
					SET_STATIC_PIXEL_SHADER( pp_teeth_bump_ps30 );
				}
#endif
			}
			else
			{
...
#endif
				{
					...
					DECLARE_STATIC_VERTEX_SHADER( pp_teeth_vs20 );
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER_COMBO( USE_STATIC_CONTROL_FLOW, bUseStaticControlFlow );
					SET_STATIC_VERTEX_SHADER( pp_teeth_vs20 );

					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_STATIC_PIXEL_SHADER( pp_teeth_ps20b );
						SET_STATIC_PIXEL_SHADER( pp_teeth_ps20b );
					}
					else
					{
						DECLARE_STATIC_PIXEL_SHADER( pp_teeth_ps20 );
						SET_STATIC_PIXEL_SHADER( pp_teeth_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
					SET_STATIC_VERTEX_SHADER( pp_teeth_vs30 );

					DECLARE_STATIC_PIXEL_SHADER( pp_teeth_ps30 );
					SET_STATIC_PIXEL_SHADER( pp_teeth_ps30 );
				}
#endif
			}
		...
		DYNAMIC_STATE
		{
			...
			{	
...
#endif
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( pp_teeth_bump_vs20 );
					...
					SET_DYNAMIC_VERTEX_SHADER( pp_teeth_bump_vs20 );
					...
					if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						...
						SET_DYNAMIC_PIXEL_SHADER( pp_teeth_bump_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_bump_ps20 );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( NUM_LIGHTS, lightState.m_nNumLights );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( AMBIENT_LIGHT, lightState.m_bAmbientLight ? 1 : 0 );
						SET_DYNAMIC_PIXEL_SHADER( pp_teeth_bump_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					SET_DYNAMIC_PIXEL_SHADER_COMBO( AMBIENT_LIGHT, lightState.m_bAmbientLight ? 1 : 0 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( pp_teeth_bump_ps30 );
				}
#endif
			}
			else
			{
...
#endif
				{
					...
					DECLARE_DYNAMIC_VERTEX_SHADER( pp_teeth_vs20 );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
					...
					SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
					SET_DYNAMIC_VERTEX_SHADER_COMBO( NUM_LIGHTS, bUseStaticControlFlow ? 0 : lightState.m_nNumLights );
					SET_DYNAMIC_VERTEX_SHADER( pp_teeth_vs20 );

					if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_ps20b );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
						SET_DYNAMIC_PIXEL_SHADER( pp_teeth_ps20b );
					}
					else
					{
						DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_ps20 );
						SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
						SET_DYNAMIC_PIXEL_SHADER( pp_teeth_ps20 );
					}
				}
#ifndef _X360
				else
				{
					...
					DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_ps30 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
					SET_DYNAMIC_PIXEL_SHADER( pp_teeth_ps30 );
				}
#endif
			}
	...
	void DrawFlashlight( IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, VertexCompressionType_t vertexCompression )
	{
...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER( pp_teeth_flashlight_vs20 );
				SET_STATIC_VERTEX_SHADER_COMBO( INTRO, params[INTRO]->GetIntValue() ? 1 : 0 );
				SET_STATIC_VERTEX_SHADER( pp_teeth_flashlight_vs20 );
				...
				{
					DECLARE_STATIC_PIXEL_SHADER( pp_teeth_flashlight_ps20 );
					SET_STATIC_PIXEL_SHADER( pp_teeth_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
		...
		DYNAMIC_STATE
		{
...
#endif
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( pp_teeth_flashlight_vs20 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( pp_teeth_flashlight_vs20 );

				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps20b );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
					SET_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps20b );
				}
				else
				{
					DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps20 );
					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
					SET_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps20 );
				}
			}
#ifndef _X360
			else
			{
				...
				DECLARE_DYNAMIC_VERTEX_SHADER( pp_teeth_flashlight_vs30 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING, bHasFastVertexTextures && pShaderAPI->IsHWMorphingEnabled() );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER( pp_teeth_flashlight_vs30 );

				DECLARE_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( UBERLIGHT, bUberlight );
				SET_DYNAMIC_PIXEL_SHADER( pp_teeth_flashlight_ps30 );
			}
#endif
